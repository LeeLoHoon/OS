...
// The following fields should be computed by ReadPageTable() from the above fields.
	//2^n 즉, offset으로 정해짐
	int page_size;
	//이 문제의 경우는 table의 length와 seg_number를 비교해 더 작은 것에 맞추어 limit을 잡음
	int limit; 
} PageTable;
...
int main(int argc, char *argv[])
{
	...
	for(int logical_addr = 0; logical_addr < addr_limit; logical_addr += step){
		int* physical_addr;
		physical_addr = Translate(logical_addr, &pt);
		printf("%04d (0x%04x) ==> %04d (0x%04x) ~ %04d (0x%04x)\n", logical_addr, logical_addr, physical_addr[0],physical_addr[1]);
	}
...
int ReadPageTable(const char *filename, PageTable *pt)
{
	...
	ptr = strtok(NULL," ");
	pt->bits_for_seg_number = atoi(ptr);
	...
	int max_seg_pages = 1<<pt->bits_for_seg_number;
	if(pt->page_size*pt->page_table_length<=max_seg_pages*pt->page_size)
		pt->limit = pt->page_table_length*pt->page_size;
	else pt->limit = max_seg_pages*pt->page_size;
	...
	fgets(buffer,sizeof(buffer),fp);
	ptr=strtok(buffer," ");
	pt->limit_number = (int *)malloc(sizeof(int));
	pt->limit_number[0] = atoi(ptr);
	ptr=strtok(NULL," ");
	pt->base_number = (int*)malloc(sizeof(int));
	pt->base_number[0] = atoi(ptr);
	...
	while(!feof(fp)){
		...
		ptr=strtok(NULL," ");
		pt->base_number = (int *)realloc(pt->base_number,(i+1)*sizeof(int));
		pt->base_number[i] = atoi(ptr);
...
{	
	...
	printf("BITS_FOR_SEG_PAGE_NUMBER %d (maximum # of seg pages = %d)\n",
		pt->bits_for_seg_number, 1 << pt->bits_for_seg_number);
	...
	for(int i = 0; i < pt->page_table_length; i++)
		printf("%3d: %d ~ %d\n", i, pt->base_number[i],(pt->base_number[i]+pt->limit_number[i]));

}

void DestroyPageTable(PageTable *pt)
{
	free(pt->limit_number);
	free(pt->base_number);
}
...
int* Translate(int logical_addr, PageTable *pt)
{
	...
	int* addr = (int*)malloc(2*sizeof(int));
	int p = logical_addr>>pt->bits_for_offset;
	int off = logical_addr%pt->page_size;
	
	int limit = pt->limit_number[p];
	
	if(off<limit){
		addr[0]=pt->base_number[p];
		addr[1]=pt->base_number[p] + pt->limit_number[p];
		return addr;
	}	 
	else return addr;
